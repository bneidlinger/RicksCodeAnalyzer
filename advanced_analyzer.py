...
class AdvancedCodeAnalyzer:
    ...
    def _analyze_python_file(self, file_path, content):
        ...
        try:
            ...
            # If we can't parse as AST, fall back to regex-based analysis
            self.update_progress(f"Syntax error in {file_path}, using regex-based analysis")
            self._analyze_python_with_regex(file_path, content)
    ...
    def _analyze_python_functions(self, tree, file_path):
        """Analyze Python functions from AST"""
        function_metrics = {}
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if params > 5:
                    self.results['code_smells'][file_path].append({
                        ...
                        'description': f"Function '{func_name}' has {params} parameters",
                        'severity': 'medium',
                        'line': node.lineno
                    })
                ...
                for inner_node in ast.walk(node):
                    if isinstance(inner_node, (ast.If, ast.While, ast.For, ast.Try, ast.AsyncFor)):
                        complexity += 1
                ...
                self._check_python_nesting(node, func_name, file_path)

        self.function_metrics[file_path] = function_metrics
    ...
    def _analyze_duplicated_code(self):
        """Analyze collected duplicated code blocks"""
        for block_hash, occurrences in self.duplicated_blocks.items():
            if len(occurrences) > 1:
                # Only report duplications across different files or non-overlapping blocks in the same file
                unique_files = set(o['file'] for o in occurrences)

                if len(unique_files) > 1 or (
                        len(unique_files) == 1 and
                        len(set((o['start_line'], o['end_line']) for o in occurrences)) > 1
                ):
                    # Simplify the report format
                    duplication = {
                        'instances': [
                            {
                                'file': o['file'],
                        ...
                        'size_lines': occurrences[0]['end_line'] - occurrences[0]['start_line'] + 1,
                        'sample': occurrences[0]['content'][:200] + '...' if len(occurrences[0]['content']) > 200 else
                        occurrences[0]['content']
                    }

                    self.results['duplicated_code'].append(duplication)

                    # Also add to code smells for each file
                    for instance in duplication['instances']:
                        self.results['code_smells'][instance['file']].append({
                            'type': 'duplicate_code',
                            'description': f"Duplicated code block ({duplication['size_lines']} lines) also found in {len(duplication['instances']) - 1} other location(s)",
                            'severity': 'high',
